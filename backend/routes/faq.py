...
def _list(search: str | None = Query(None), page: int = 1, page_size: int = 50, db: Session = Depends(get_db)):
...
def _delete(faq_id: int, db: Session = Depends(get_db)):
    ...
    if not ok: raise HTTPException(404, "FAQ not found")
    return {"ok": True}
